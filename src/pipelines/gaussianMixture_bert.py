...
class GaussianMixtureBertPipeline(ExperimentPipeline):
    """Pipeline: Fasttext -> GaussianMixture -> evaluation/interpretation.
    ...
    def _run_single_trial(
        ...
        pipeline_result = PipelineResult(
            ...
            interpretation=None,
            metadata={"pipeline": "gaussianMixture_fasttext", "n_components": n_components, "random_state": random_state},
        )
    ...
    def run_many(
        ...
        if best_idx is not None:
            ...
            probs = None
            if best_result.clustering.probabilities is not None:
                print("got probs")
                probs = best_result.clustering.probabilities.detach().cpu().tolist()

            labels_list = best_result.clustering.labels.detach().cpu().tolist()
            ...
            for i, (document, label) in enumerate(zip(documents, labels_list)):
                ...
                if probs is not None:
                    entry["probabilities"] = probs[i]
                    lbl_idx = int(label)
                    entry["cluster_probability"] = float(probs[i][lbl_idx]) if 0 <= lbl_idx < len(probs[i]) else None
                else:
        ...
        return MultiRunPipelineResult(
            ...
            metadata={"pipeline": "gaussianMixture_fasttext", "n_trials": n_trials, "optuna_seed": 42, "scoring": "silhouette, calinski_harabasz, davies_bouldin"},
        )
