...
from pipelines.agglomerative_fasttext import AgglomerativeFasttextPipeline
...
class AgglomerativeExperiment(BaseExperiment[ParsedExperimentConfig]):
    """Encapsulates the Agglomerative + Fasttext experiment logic."""
    ...
    def build_pipeline(self) -> ExperimentPipeline:
        ...
        return AgglomerativeFasttextPipeline(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Fasttext + Agglomerative experiment")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    experiment = AgglomerativeExperiment(config_path)
    experiment.run()
