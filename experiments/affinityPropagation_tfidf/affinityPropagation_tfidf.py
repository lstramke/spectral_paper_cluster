...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run TF-IDF + AffinityPropagation experiment")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    # Per-field checks so the type checker knows values are non-None.
    ...
    if parsed.affinityPropagation is None:
        print("Missing required config: affinityPropagation", file=sys.stderr)
        sys.exit(1)
    ...
    pipeline = AffinityPropagationTfidfPipeline(
        affinityPropagation_config=experimentConfig.affinityPropagation,
