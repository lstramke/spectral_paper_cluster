...
from src.pipelines.optics_tfidf import OpticsTfidfPipeline
...
class OpticsExperiment(BaseExperiment[ParsedExperimentConfig]):
    """Encapsulates the OPTICS + TF-IDF experiment logic."""
    ...
    def load_config(self) -> None:
        ...
        if parsed.optics is None:
            raise ValueError("Missing required config: optics")
    ...
    def run(self) -> None:
        ...
        pipeline = OpticsTfidfPipeline(
            optics_config=self.experiment_config.optics,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run TF-IDF + OPTICS experiment")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    experiment = OpticsExperiment(config_path)
    experiment.run()
