...
from pipelines.spectral_tfidf import SpectralTfidfPipeline
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run TF-IDF + SpectralClustering experiment")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    if parsed.spectral is None:
        print("Missing required config: spectral", file=sys.stderr)
        sys.exit(1)
    ...
    pipeline = SpectralTfidfPipeline(
        spectral_config=experimentConfig.spectral,
