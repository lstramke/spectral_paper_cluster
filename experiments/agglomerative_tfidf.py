...
from pipelines.agglomerative_tfidf import AgglomerativeTfidfPipeline
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run TF-IDF + Agglomerative experiment")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    if parsed.agglomerative is None:
        print("Missing required config: agglomerative", file=sys.stderr)
        sys.exit(1)
    ...
    pipeline = AgglomerativeTfidfPipeline(
        agglomerative_config=experimentConfig.agglomerative,
